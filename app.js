...
async function fetchMany(paths, label) {
  ...
  if (!ok.length) {
    throw new Error(`Nenhum arquivo ${label} carregou. Falharam: ${failed.join(", ")}`);
  }

  if (failed.length) {
    console.warn(`Alguns arquivos ${label} falharam:`, failed);
  }
...
async function loadData() {
  try {
    ...
    loadingBox.textContent = "Carregando manhwas...";

    const [catalog, chapterData] = await Promise.all([
      fetchMany(MANHWA_PARTS, "de catálogo"),
      fetchMany(MANGA_PARTS, "de capítulos")
    ]);

    const chaptersByUrl = new Map(chapterData.map(item => [normalizeUrl(item.url), item]));

    MANHWAS = catalog.map((item, index) => {
      const info = chaptersByUrl.get(normalizeUrl(item.url)) || {};
      let chapters = [];

      if (Array.isArray(info.capitulos)) {
        chapters = [...info.capitulos];
      } else if (info.total) {
        chapters = Array.from({ length: Number(info.total) }, (_, i) => i + 1);
      }
    ...
    loadingBox.innerHTML = `Erro ao carregar os arquivos JSON.<br><br><small>${error.message}</small>`;
    console.error(error);
  }
}
...
function openDetails(id) {
  ...
  detailsBox.innerHTML = `
    ...
    <div class="chapters">${chapters.length ? chapters.map((cap, index) => `<button class="chapterBtn" onclick="openReader(${index})">${chapterTitle(cap)}</button>`).join("") : `<div class="empty">Nenhum capítulo cadastrado ainda.</div>`}</div>`;
...
function renderReader() {
  ...
  chapterSelect.innerHTML = chapters.map((cap, index) => `<option value="${index}" ${index === currentChapterIndex ? "selected" : ""}>${chapterTitle(cap)}</option>`).join("");

  const images = chapterImages(chapter).filter(Boolean);
  const originalUrl = originalChapterUrl(currentManhwa, chapter);

  if (images.length) {
    readerImages.innerHTML = images.map((src, i) => `<img src="${src}" alt="Página ${i + 1}" loading="lazy" referrerpolicy="no-referrer">`).join("");
  } else {
    readerImages.innerHTML = `<div class="empty readerEmpty"><h3>${chapterTitle(chapter)}</h3><p>Este JSON tem o número do capítulo, mas ainda não tem os links das imagens internas.</p><p>Quando você adicionar as imagens dentro de <code>mangas_resultado.json</code>, o leitor vai exibir tudo aqui automaticamente.</p><a class="sourceBtn" href="${originalUrl}" target="_blank" rel="noopener">Abrir capítulo original</a></div>`;
  }
...
function filterManhwas() {
  ...
  return MANHWAS.filter(m => m.nome.toLowerCase().includes(term) || String(m.status || "").toLowerCase().includes(term));
}
